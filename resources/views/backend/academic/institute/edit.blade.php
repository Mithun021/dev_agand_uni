    ...
    <div class="col-sm-12 col-md-12">
        <div class="card">
            @if($errors->any())
                ...
                @endif
            <div class="card-header pb-0"><h4>Edit Institute</h4></div>
            <form class="form theme-form" method="POST" action="{{ route('institutes.update',$institute->id) }}">
                @csrf
                @method('PUT')
            <div class="card-body">
                @if(session('message'))
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="position-relative input-icon">
                            <select name="is_active" class="form-control @error('is_active') is-invalid @enderror" id="input14" required>
                                <option value="" disabled {{ old('is_active', $institute->is_active) === '' ? 'selected' : '' }}>Select Status</option>
                                <option value="1" {{ old('is_active', $institute->is_active) == '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $institute->is_active) == '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
